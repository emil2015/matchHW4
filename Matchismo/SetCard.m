...
- (NSString *)contents
{
    ...
    if (self.count == 1) {
        titleString = [NSString stringWithFormat:@"%@", self.symbol];
...
{
    NSMutableAttributedString *attributedContents = [[NSMutableAttributedString alloc] initWithString:self.contents];
    
    UIColor *foregroundColor = [self color];
    UIColor *strokeColor = [foregroundColor copy];
    foregroundColor = [foregroundColor colorWithAlphaComponent:self.shade];
    
    [attributedContents setAttributes:@{NSForegroundColorAttributeName:foregroundColor,
                                        NSStrokeWidthAttributeName:@-5,
                                        NSStrokeColorAttributeName:strokeColor}
                                range:NSMakeRange(0, [attributedContents length])];
...
{
    NSNumber *number = [[NSNumber alloc] initWithFloat:shade];
    if ([[SetCard validShades] containsObject:number]) {
        _shade = shade;
    }
}
